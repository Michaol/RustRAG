...
v2.4.0 新增多格式文档支持，将 RustRAG 从代码索引引擎升级为通用文档 RAG 引擎：

- **多格式文档支持**：支持索引纯文本（`.txt`、`.log`）、结构化数据（`.json`、`.yaml`、`.yml`、`.toml`、`.csv`）、HTML（`.html`、`.htm`）、PDF（`.pdf`）、Word（`.docx`）、电子表格（`.xls`、`.xlsx`、`.xlsb`、`.ods`）。
- **格式专属分块策略**：每种格式使用定制化的提取和分块策略，保留结构信息（JSON 键路径、CSV 表头、电子表格 Sheet 名称等）。
- **可配置扩展名**：`config.json` 中默认启用全部 24 种文件类型，用户可删除扩展名来过滤不需要的格式，完全支持配置热加载。
- **新增依赖**（全部纯 Rust，无 C 绑定）：`lopdf`（PDF）、`docx-rs`（DOCX）、`calamine`（XLS/XLSX/ODS）、`scraper`（HTML）、`toml`、`csv`。
- **其他**：将 `.jsx`/`.tsx` 补入支持的代码扩展名（Tree-sitter 已支持）。

---

<details>
<summary><b>展开查看历史演进 (v2.3.0 及更早版本)</b></summary>
<br>

### v2.3.0 安全加固与代码质量

v2.3.0 是安全加固与代码质量提升版本，通过系统化代码审查修复了 26 项问题：

- **安全修复**：修复 Windows 路径校验漏洞，限制 MCP 工具的任意文件读取，HTTP 服务默认绑定 localhost。
- **可靠性提升**：将生产环境 `assert_eq!` panic 替换为正确的错误传播，修复索引计数器逻辑，阻塞下载任务包装为 `spawn_blocking`。
- **配置增强**：无效 JSON 配置现在返回错误而非静默回退默认值；启动时校验向量维度与 sqlite-vec schema 一致性。
- **国际化支持**：语言检测新增日语（平假名/片假名）和韩语（韩文）识别；YAML frontmatter 正确转义特殊字符。
- **性能优化**：ONNX 线程数通过 `available_parallelism()` 自动检测；`LanguageConfig` 使用 `LazyLock` 缓存；`build_dictionary` 默认限制遍历 100 篇文档。
- **代码质量**：移除 PHP 死代码路径，修复文件监听器 TOCTOU 竞态，为 unsafe 块添加 `// SAFETY:` 文档。

<details>
<summary><b>展开查看历史演进 (v2.2.0 及更早版本)</b></summary>
<br>

### v2.2.0 架构深度重构

v2.2.0 引入了针对高并发和异步可靠性的架构深度重构：

- **数据库连接池**：引入 `r2d2` 封装 `sqlite-vec`，实现了安全、高性能的高并发 SQLite 访问能力。
- **纯异步网络 I/O**：将版本检查器从 `reqwest::blocking` 迁移至原生异步 `reqwest`，彻底解决了潜在的 Tokio 线程饥饿问题。
- **配置加载安全性**：消除了配置加载过程中的 TOCTOU（时间差检查）竞态条件，增强了文件系统交互的健壮性。
- **性能优化**：优化了 ONNX 嵌入引擎的惰性初始化流程，并改进了全局错误冒泡处理机制。

<details>
<summary><b>展开查看历史演进 (v2.1.0 及更早版本)</b></summary>
<br>

### v2.1.0 高级功能改进

v2.1.0 引入了高级功能和改进，增强了性能、可靠性和开发者体验：
...
- **24 种支持格式** — 代码（Rust, Go, Python, TypeScript, JavaScript）、Markdown、纯文本、结构化数据（JSON, YAML, TOML, CSV）、HTML、PDF、DOCX、电子表格（XLS, XLSX, XLSB, ODS）。
...
1. **下载官方 GPU 运行时**
   前往 [ONNX Runtime v1.25.1 Release 页面](https://github.com/microsoft/onnxruntime/releases/tag/v1.25.1)，根据您的系统下载带 GPU 支持的压缩包（体积约 300+MB）：

- **Windows:** 下载 `onnxruntime-win-x64-gpu-1.25.1.zip`
- **Linux:** 下载 `onnxruntime-linux-x64-gpu-1.25.1.tgz`
